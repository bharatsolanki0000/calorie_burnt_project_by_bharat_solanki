...
age= st.slider("Enter your age ", min_value=10, max_value=90, step=1, value=20)
gender=st.radio("Choose your gender",["Male","Female"])
heart_rate = st.slider(
    "Enter heartbeat rate in bpm",
    min_value=50,
    max_value=200,
    value=90,
    step=1
...
body_temp = st.slider(
    "Enter body temperature in Celsius",
    min_value=36.0,
    max_value=40.0,
    value=36.5,
    step=0.1
...
duration=st.slider("Enter duration of Exercise in minutes",min_value=1,max_value=120,value=20, step=1)
height = st.slider(
...
target_calories=st.slider("enter target calorie you want to burn!",min_value=1.0,max_value=314.0,value=100.0, step=.1)
...
if calculate_what_if:
    if (not name):
        ...
        st.info(f"You would need to exercise for approx **{new_duration:.2f} minutes** to reach your goal, keeping all other factors the same.")
